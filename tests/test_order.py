...
def test_order_with_top_button(driver, name, surname, address, metro, phone, date, comment):
...
@pytest.mark.parametrize("name, surname, address, metro, phone, date, comment", test_data)
def test_order_with_bottom_button(driver, name, surname, address, metro, phone, date, comment):
    main_page = MainPage(driver)
    order_page = OrderPage(driver)
